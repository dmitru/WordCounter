 ...
 * Дополнительно: разобраться с использованием регулярных выражений в Java<br/>
 *
 * ЗАДАНИЕ<br/>
 ...
 *
 * ПРИМЕЧАНИЕ<br/>
 * Для тестирования можно текст взять, например, из http://www.gnu.org/licenses/lgpl-3.0.txt.
 *
...
public interface WordCounter
{
     ...
     * Все возвраемые слова должны быть приведены к нижнему регистру.
     ...
     * и упорядоченный в прядке убывания количества вхождений слова.
     * Слова с одинаковым количеством вхождений упорядочиваются в алфавитном порядке (без учета регистра!).
     * Все возвраемые слова должны быть приведены к нижнему регистру.
    ...
    /**
     * Упорядочивает результат подсчета количеств вхождений слов в порядке
     * убывания количества вхождений, причем слова с одинаковыми количествами
     * вхождений упорядочиваются в алфавитном порядке (без учета регистра!).
     * Реализация этого метода не является обязательной.
     ...
     * </ul>
     * Все выводимые слова должны быть приведены к нижнему регистру.
     * Метод не является автоматически тестируемым, но мы узнаем, если вы его не реализуете.
     ...
     * </ul>
     * Все выводимые слова должны быть приведены к нижнему регистру.
     * Реализация данного метода не является обязательной.
     * Метод не является автоматически тестируемым, но мы узнаем, если вы его не реализуете.
     * Проверить корректность метода (для себя, в методе main) вы можете, передав в него System.out.
     * @param ps поток вывода
     * @throws IllegalStateException если не задан текст для анализа
